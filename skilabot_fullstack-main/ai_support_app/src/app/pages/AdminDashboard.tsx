...
function StatCard({
  ...
  return (
    <div
      className="p-5 rounded-2xl bg-white border flex flex-col gap-3"
...
function CT({ active, payload, label }: any) {
  ...
  return (
    <div
      className="px-3 py-2 rounded-xl shadow-lg text-xs"
      style={{
        background: "#fff",
        border: "1px solid rgba(124,58,237,0.15)",
      }}
    >
      <p className="font-semibold mb-1" style={{ color: "#12082a" }}>
        {label}
      </p>
...
function SentimentBadge({ sentiment }: { sentiment: string }) {
  ...
  return (
    <span style={{ color: s.color }} className="text-sm">
      {s.emoji} <span className="text-xs capitalize">{sentiment}</span>
    </span>
  );
}
...
export default function AdminDashboard() {
  ...
  // IMPORTANT: this directly stores conversations from backend
  const [adminConversations, setAdminConversations] = useState<ApiConversation[]>(
    []
  );
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [sum, sett, convs] = await Promise.all([
        api.adminSummary(),
        api.adminSettings(),
        api.adminConversations(),
      ]);

      const fixedConvs = convs.map((c) => ({
        ...c,
      ...
      setAdminConversations(fixedConvs);

      setSummary({
        ...sum,
        totalConversations: fixedConvs.length,
        recentConversations: fixedConvs,
        users: sum.users ?? sum.totalUsers ?? 0,
        leads: sum.leads ?? sum.leadsCaptured ?? 0,
        escalated: sum.escalated ?? 0,
        resolved: sum.resolved ?? 0,
        open:
          sum.open ??
          fixedConvs.filter((conversation) => conversation.status === "open")
            .length,
        resolutionRate: sum.resolutionRate ?? 0,
        sentiment: sum.sentiment ?? {
          positive: 0,
          neutral: fixedConvs.length,
          negative: 0,
        },
      });
  ...
  function handleExport() {
    ...
    fetch(url, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then((blob) => {
        ...
        a.download = `conversations-${new Date()
          .toISOString()
          .slice(0, 10)}.csv`;
  ...
  async function handleDeleteConversation(id: string) {
    ...
    try {
      ...
      setAdminConversations((prev) => prev.filter((c) => c.id !== id));
      ...
      setSummary((prev) =>
        prev
          ? {
              ...
              recentConversations: (prev.recentConversations || []).filter(
                (c) => c.id !== id
              ),
            }
          : prev
  ...
  async function handleViewConversation(id: string) {
    ...
    try {
      ...
      const localConversation = adminConversations.find((c) => c.id === id);
  ...
  const liveStats = summary
    ? [
        ...
        {
          label: "Escalated",
          value: String(summary.escalated ?? 0),
          delta: (summary.escalated ?? 0) > 5 ? "+2" : "-1",
          up: (summary.escalated ?? 0) <= 5,
          icon: AlertTriangle,
          color: "#ef4444",
        },
        {
          label: "Total users",
          value: String(summary.users ?? summary.totalUsers ?? 0),
          delta: "+8%",
          up: true,
          icon: Users,
          color: "#f97316",
        },
        {
          ...
          value: String(summary.leads ?? summary.leadsCaptured ?? 0),
          delta: "+15%",
          up: true,
          icon: Star,
          color: "#06b6d4",
  ...
  return (
    ...
    >
      ...
      >
        {(["overview", "conversations", "analytics", "settings"] as Tab[]).map(
          (t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
            ...
            >
              <table className="w-full text-sm">
                <thead>
                  ...
                  >
                    {["Title", "Status", "Sentiment", "Messages", "Updated", "Actions"].map(
                      (h) => (
                        <th
                          key={h}
                ...
                <tbody>
                  {filteredConvs.map((c, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <SentimentBadge sentiment={c.sentiment || "neutral"} />
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="relative flex items-center gap-1">
                          <button
                            onClick={() =>
                              setConvMenuOpen(convMenuOpen === c.id ? null : c.id)
                            }
        ...
        <div className="space-y-6">
          ...
          {summary && (
            <div className="grid grid-cols-3 gap-4">
              <div
                className="p-5 rounded-2xl bg-white border text-center"
                style={{ borderColor: "rgba(16,185,129,0.2)" }}
              >
                <p className="text-3xl font-extrabold" style={{ color: "#10b981" }}>
                  {summary.sentiment?.positive ?? 0}
                </p>
                <p className="text-sm mt-1" style={{ color: "#7a6080" }}>
          ...
          >
            ...
            <p className="text-sm mb-4" style={{ color: "#7a6080" }}>
              Download all conversations as a CSV file for analysis in Excel or Google
              Sheets.
            </p>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSettings((s) => ({ ...s, botTone: e.target.value }))
                  }
                  className="w-full px-4 py-2.5 rounded-xl text-sm outline-none border transition-colors focus:border-[#7c3aed]"
                  style={{
                    background: "#fef9ff",
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="text-xs mb-2" style={{ color: "#7a6080" }}>
                  Bot escalates when confidence drops below this level
                </p>
              ...
              <div>
                ...
                <p className="text-xs mb-2" style={{ color: "#7a6080" }}>
                  After N turns without resolution, escalate to a human
                </p>
          ...
          >
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="text-xs mb-2" style={{ color: "#7a6080" }}>
                  Receives an email every time a conversation is escalated
                </p>
              ...
              <div>
                ...
                <p className="text-xs mb-2" style={{ color: "#7a6080" }}>
                  Post escalation summaries to a Slack channel
                </p>
      ...
      {selectedConversation && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="w-full max-w-3xl max-h-[85vh] bg-white rounded-2xl shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div>
                ...
                <p className="text-xs" style={{ color: "#7a6080" }}>
                  Status: {selectedConversation.status || "open"} · Sentiment:{" "}
            ...
            <div
              className="p-6 overflow-y-auto space-y-4 max-h-[65vh]"
              style={{ background: "#fef9ff" }}
            >
